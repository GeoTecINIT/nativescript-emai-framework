...
export interface LocalTimeSeriesStore<T extends TimeSeriesEntity>
...
export abstract class AbstractTimeSeriesStore<T extends TimeSeriesEntity>
  implements LocalTimeSeriesStore<T> {
  private readonly store: EMAIStore<T & DBEntityProps>;
  ...
  protected constructor(
    ...
    this.store = new EMAIStore<T & DBEntityProps>(
      docType,
      serializer,
      deserializer
    );
  ...
  private async getDBEntityFrom(entity: T): Promise<T & DBEntityProps> {
    ...
    const where = Object.keys(serializedEntity).map((key, index) => {
      const conditional =
        index > 0 ? { logical: QueryLogicalOperator.AND } : {};
      return {
        ...conditional,
        property: key,
        comparison: "equalTo",
  ...
  private removeDBEntityPropsFromAll(
    dbEntities: Array<T & DBEntityProps>
  ): Array<T> {
